...
def apply_templates(X, templates):
    ...
    for template in templates:
        name = u('|').join([u('%s[%d]') % (f, o) for f, o in template])
        X_len = len(X)
...
def readiter(data, names):
    ...
    for sentence in data:
        ...
        for fields in sentence:
            # print(u'{}\t{}'.format(*fields))
            # fields = line.split(sep)
            if len(fields) != len(names):
                raise ValueError(
                    'Too few fields (%d) for %r\n%s' % (len(fields), names, sentence))
...
def main(data, feature_extractor, fields):
    # fi = sys.stdin
    # fo = sys.stdout
    ...
    for X in readiter(data, F):
        feature_extractor(X)
        sen_feature = []
        for x in X:
            # pp.pprint(x['F'])
            sen_feature.append(x['F'])
